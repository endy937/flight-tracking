...
const Utils = {
    ...
    valid(sentence, validateChecksum) {
    ...
    appendChecksum(sentence) {
        ...
        if (split.length === 1) {
            if (split[0].charAt(0) == '$' || split[0].charAt(0) == '!') {
                return split[0].concat('*').concat(this.checksum(split[0]).toString(16).padStart(2, '0').toUpperCase());
            }
        }
        return sentence;
    },
    ...
    transform(value, inputFormat, outputFormat) {
        ...
        if (inputFormat == 'km') {
            if (outputFormat == 'nm')
                return value / this.RATIOS.NM_IN_KM;
        }
        // NM
        if (inputFormat == 'nm') {
            if (outputFormat == 'km')
                return value / this.RATIOS.KM_IN_NM;
            if (outputFormat == 'm')
                return value * 1000 / this.RATIOS.KM_IN_NM;
        }
        // KNOTS
        if (inputFormat == 'knots') {
            if (outputFormat == 'kph')
                return value / this.RATIOS.KPH_IN_KNOTS;
            if (outputFormat == 'ms')
                return value / this.RATIOS.MS_IN_KNOTS;
            if (outputFormat == 'mph')
                return value / this.RATIOS.MPH_IN_KNOTS;
        }
        // KPH
        if (inputFormat == 'kph') {
            if (outputFormat == 'knots')
                return value / this.RATIOS.KNOTS_IN_KPH;
            if (outputFormat == 'ms')
                return value / this.RATIOS.MS_IN_KPH;
            if (outputFormat == 'mph')
                return value / this.RATIOS.MPH_IN_KPH;
        }
        // MPH
        if (inputFormat == 'mph') {
            if (outputFormat == 'knots')
                return value / this.RATIOS.KNOTS_IN_MPH;
            if (outputFormat == 'ms')
                return value / this.RATIOS.MS_IN_MPH;
            if (outputFormat == 'kph')
                return value / this.RATIOS.KPH_IN_MPH;
        }
        // MS
        if (inputFormat == 'ms') {
            if (outputFormat == 'knots')
                return value / this.RATIOS.KNOTS_IN_MS;
            if (outputFormat == 'mph')
                return value / this.RATIOS.MPH_IN_MS;
            if (outputFormat == 'kph')
                return value / this.RATIOS.KPH_IN_MS;
        }
        // ANGLES
        if (inputFormat == 'deg') {
            if (outputFormat == 'rad')
                return value / this.RATIOS.RAD_IN_DEG;
        }
        if (inputFormat == 'rad') {
            if (outputFormat == 'deg')
                return value / this.RATIOS.DEG_IN_RAD;
        }
        if (inputFormat == 'c') {
            if (outputFormat == 'k')
                return value + this.RATIOS.CELCIUS_IN_KELVIN;
        }
        if (inputFormat == 'k') {
            if (outputFormat == 'c')
                return value - this.RATIOS.CELCIUS_IN_KELVIN;
        ...
        if (inputFormat == 'ft') {
            if (outputFormat == 'm')
                return value / this.RATIOS.METER_IN_FEET;
        }
        if (inputFormat == 'fa') {
            if (outputFormat == 'm')
                return value / this.RATIOS.METER_IN_FATHOM;
        }
    ...
    timestamp(time, date) {
        ...
        if (time) {
            hours = this.int(time.slice(0, 2));
            minutes = this.int(time.slice(2, 4));
            seconds = this.int(time.slice(4, 6));
        }
        else {
            var dt = new Date();
        ...
        if (date) {
            var year, month, day;
            day = this.int(date.slice(0, 2));
            month = this.int(date.slice(2, 4));
