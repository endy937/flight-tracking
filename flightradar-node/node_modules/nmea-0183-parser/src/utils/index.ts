const Utils = {
    ...
    valid(sentence: string, validateChecksum: number) {
    ...
    transform(value: number, inputFormat: string, outputFormat: string) {
        ...
        if (inputFormat == 'nm') {
            ...
            if (outputFormat == 'm') return value * 1000 / this.RATIOS.KM_IN_NM;
        }
        ...
        // ANGLES
        if (inputFormat == 'deg') {
            if (outputFormat == 'rad') return value / this.RATIOS.RAD_IN_DEG;
        }

        if (inputFormat == 'rad') {
            if (outputFormat == 'deg') return value / this.RATIOS.DEG_IN_RAD;
        }

        if (inputFormat == 'c') {
            if (outputFormat == 'k') return value + this.RATIOS.CELCIUS_IN_KELVIN;
        }

        if (inputFormat == 'k') {
            if (outputFormat == 'c') return value - this.RATIOS.CELCIUS_IN_KELVIN;
            if (outputFormat == 'f') return (1.8 * (value - 273)) + 32;
        }
        // LENGTH
        if (inputFormat == 'ft') {
            if (outputFormat == 'm') return value / this.RATIOS.METER_IN_FEET;
        }

        if (inputFormat == 'fa') {
            if (outputFormat == 'm') return value / this.RATIOS.METER_IN_FATHOM;
        }
        // Just return input if input/output formats are not recognised.
        return value;
    },

    timestamp(time: string, date: string) {
        /* TIME (UTC) */
    ...
    coordinate(value: string, pole: string) {
