...
@section('title', 'Follow Aircraft Data')

@section('contents')
    <div style="width: 100%; height: 100%;">
        <div class="mb-3">
            <h2>Table Follow Aircrat</h2>
        </div>
        <table class="min-w-full border border-gray-200 text-sm text-left">
            <thead class="bg-gray-100 font-bold">
                <tr>
                    ...
                    <th class="border px-4 py-2">Registation</th>
                    <th class="border px-4 py-2">Icao24Bit</th>
            ...
            <tbody>
                @foreach ($data as $item)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="border px-4 py-2">{{ $item->created_by ?? '-' }}</td>
                        <td class="border px-4 py-2">{{ $item->callsign ?? '-' }}</td>
                        <td class="border px-4 py-2">{{ $item->lat ?? '-' }}</td>
                        <td class="border px-4 py-2">{{ $item->lon ?? '-' }}</td>
                        <td class="border px-4 py-2">{{ $item->registration ?? '-' }}</td>
                        <td class="border px-4 py-2">{{ $item->icao24bit ?? '-' }}</td>
                        <td class="border px-4 py-2">
                            <div class="flex justify-center items-center gap-5">
                                <a href="{{ route('follow_delete', $item->id) }}" data-confirm-delete="true">
